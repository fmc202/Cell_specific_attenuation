...
@njit
@cc.export("compute_code", "i8(f8, f8, f8, f8, f8, f8)")
...
@njit
@cc.export("clip_cell", "f8(f8, f8, f8, f8, f8, f8, f8, f8)")
...
@cc.export("get_fraction", "Tuple((f8[:,:], f8[:,:]))(f8[:], f8[:], f8[:], f8[:], f8[:], f8[:], f8[:])")
def get_fraction(x, y, X1, Y1, X2, Y2, R):
    """compute length in all cells for all data
    Warning!!!: input must be float64 to avoid any error; Conver scalar to np.array([scalar])
